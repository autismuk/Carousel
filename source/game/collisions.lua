...
function CollisionManager:getCollisions(gap)
	...
	local objectArray = {}
	for _,obj in pairs(carouselTable) do objectArray[#objectArray+1] = obj end 		-- convert from a hash to an array as it is quicker.
	for testIndex = 1,#objectArray-1 do 											-- for each in the table, except the last one
		...
		for altIndex = testIndex+1,#objectArray do  								-- compare against every other one.
			...
			local minDist = obj1.m_radius + obj2.m_radius + gap						-- the minimum distance between centres for a collision
			if math.abs(obj1.m_x-obj2.m_x) <= minDist then 							-- check if they *might* collide, intersecting boxes first as its quicker.
				if math.abs(obj1.m_y-obj2.m_y) <= minDist then 
					local dx = obj1.m_x - obj2.m_x 									-- now do the proper calculation. 
					local dy = obj1.m_y - obj2.m_y
